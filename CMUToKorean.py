...
from CMUKorean import CMUKorean
import abc
import sys
import re
        ...
        class IsExistsLatestKorean(Interface):
            ...
            def test(self, map, map_index, lhs):
                ...
                for i in range(1, len(lhs) + 1):
                    if re.search(str(r'[ㄱ-ㅎㅏ-ㅣ가-힣]+'), lhs[-i]):
                        found = True
                        break
    ...
    def _assembly(src_korean_list, **kwargs):
        ...
        def syllable_join(dest, src_onset, src_nucleus, src_coda):
            ...
            for o in src_onset:
                for n in src_nucleus:
                    for c in src_coda:
                        r.append(str(CMUKorean.Syllable(phoneme_onset=o, phoneme_nucleus=n, phoneme_coda=c)))
        ...
        for text in src_korean_list:
            ...
            while i < korean_len:
                ...
                if isinstance(cur, CMUKorean.Syllable):
                    # op code 라면
                    if op:
                        # 해당 자음은 무조건 밑받침
                        if op == u'~':
                            ...
                            if not cur.is_completed() and not cur.phoneme_nucleus:
                                ...
                                if not carry_onset and not carry_nucleus:
                                    ...
                                    for j in range(0, len(combination)):
                                        last_elem = CMUKorean.Syllable(letter=combination[j][-1])
                        ...
                        # 캐리와 합성
                            ...
                            if carry_nucleus and cur.phoneme_nucleus:
                                ...
                                if isinstance(carry_nucleus, list):
                                    for i in range(0, len(carry_nucleus)):
                                        ...
                                        if carry_nucleus[i] != cur.phoneme_nucleus:
                                            ...
                                            if nucleus not in CMUKorean.phoneme_nucleus_phonetic_combine_dict:
                                                carry_nucleus[i] = cur.phoneme_nucleus
                                            else:
                                                carry_nucleus[i] = CMUKorean.phoneme_nucleus_phonetic_combine_dict[nucleus]
                                ...
                                # 캐리의 중성과 현재의 중성이 일치하지 않으면
                                    ...
                                    carry_nucleus = CMUKorean.phoneme_nucleus_phonetic_combine_dict[nucleus]
        ...
        for text in pass1_result:
            ...
            for i in range(0, korean_len):
                ...
                if not isinstance(cur, CMUKorean.Syllable):
                    continue
                ...
                sound_syllable = CMUKorean.Syllable(phoneme_onset=cur.phoneme_onset,
                                                 phoneme_nucleus=cur.phoneme_nucleus,
                                                 phoneme_coda=sound_coda)

                # ㅎ,ㅅ,ㅆ,ㅈ,ㅊ 은 받침으로 불가능
                if cur.phoneme_coda in (u'ㅎ', u'ㅅ', u'ㅆ', u'ㅈ', u'ㅊ'):
                    syllable1 = CMUKorean.Syllable(phoneme_onset=cur.phoneme_onset,
                                                phoneme_nucleus=cur.phoneme_nucleus,
                                                phoneme_coda=None)

                    syllable2 = CMUKorean.Syllable(phoneme_onset=cur.phoneme_coda,
                                                phoneme_nucleus=u'ㅡ',
                                                phoneme_coda=None)
